...
"""
Creates a new library path next to the current one
"""
def create_library_path(gui, name):
    lp = get_library_path(gui)
    bp, tail = os.path.split(lp)
    np = os.path.join(bp, name)
    i = 1
    while os.path.exists(np):
        np = os.path.join(bp, "%s-%s" % (name, str(i)))
        i = i + 1
    return np
